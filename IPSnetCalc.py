...
#Given CIDR/prefix notation, converts to the other
def snMaskConverter(subnet_mask):
    #Converts CIDR to prefix notation
    if "." in subnet_mask:
        subnet = subnet_mask.split(".")
        prefix_sum = 0
        for octet in subnet:
            prefix_sum += get_key(int(octet))
        converted_subnet = ("/" + str(prefix_sum))
    #Converts prefix to CIDR notation
    if "/" in subnet_mask:
...
def numHosts(subnet_mask):
    ...
    host_bits= 32-int(subnet_mask[1:])
...
def getNetBcastBin(ip_addr, subnet_mask):
    ...
    subnet_mask = int(prefixMask(subnet_mask)[1:])
    net_addr = ''
    bcast_addr = ''
...
#gets the network address decimal
def netAddrDec(ip_addr, subnet_mask):
    bin_addr = getNetBcastBin(ip_addr,subnet_mask)[0]
    return binToDec(bin_addr)


#gets the broadcast address decimal
def bcastAddrDec(ip_addr, subnet_mask):
    bin_addr = getNetBcastBin(ip_addr,subnet_mask)[1]
    return binToDec(bin_addr)


#given network and bcast decimal, returns list of first and lat address in usable range
def usableRange(ip_addr, subnet_mask):
    net_dec = netAddrDec(ip_addr,subnet_mask).split(".")
    bcast_dec = bcastAddrDec(ip_addr,subnet_mask).split(".")
...
def IPsubnetCalculator():
    ...
    net = netAddrDec(ip_addr, subnet_mask)
    bcast = bcastAddrDec(ip_addr, subnet_mask)
    usable = usableRange(ip_addr,subnet_mask)
